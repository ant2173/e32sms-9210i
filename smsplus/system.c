...
/* cart, input, bitmap now live in the heap state struct (sms_state.h). */
...
void system_frame(int skip_render)
{
    ...
    int iline;
    extern unsigned int ProfNow(void);        /* User::TickCount() bridge */
    extern void ProfZ80Add(unsigned int);     /* accumulate into AppUi heap members */
    extern void ProfRenderAdd(unsigned int);
    ...
    for(vdp.line = 0; vdp.line < lpf;)
    {
        ...
        z80_execute(227);
        _pt1 = ProfNow();
        ProfZ80Add(_pt1 - _pt0);

       if(!skip_render)
{
    if(vdp.line < bitmap.viewport.h)
    {
        _pt0 = ProfNow();
        render_line(vdp.line);
        _pt1 = ProfNow();
        ProfRenderAdd(_pt1 - _pt0);
    }
}
