...
/* Global variables -- now live in the heap state struct (EKA1: no writable
   static data in a DLL). While sms_state.h is defining the struct itself,
   SMS_STATE_DEFINING is set and the macros stay inert so the member names
   below remain real identifiers. */
