...
/* text_counter, tms_lookup, mc_lookup, txt_lookup, bp_expand, tms_obj_lut,
   sprites, sprites_found now live in the heap state struct (sms_state.h).
   The tms_sprite type is also defined there. */
...
void render_obj_tms(int line)
{
    ...
    for(i = 0; i < sprites_found; i++)
    {
        p = &sprites[i];
        lb = &linebuf[p->xpos];
        olut = &tms_obj_lut[(p->attr & 0x0F) << 8];
